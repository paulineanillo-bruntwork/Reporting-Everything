...
// Get deals with sales_agent + collaborators for a batch of deal IDs
async function getDealsWithCollaborators(dealIds) {
  var map = {}; // dealId -> { sales_agent, collaborators: [ownerIds] }
...
// TEMP: Simulation endpoint for collaborator FTE split
app.get('/api/simulate-split', async function(req, res) {
  try {
    ...
    // 4. Resolve collaborator owner IDs to names
    var allOwnerIds = [];
    ...
    for (var ti = 0; ti < ticketIds.length; ti++) {
      ...
      for (var di = 0; di < dealIds.length; di++) {
        ...
        // Current: all FTE to sales_agent
        if (agent) currentByAgent[agent] = (currentByAgent[agent] || 0) + fte;

        // Split: divide between agent + collaborators
        if (collabs.length > 0) {
          var people = [];
